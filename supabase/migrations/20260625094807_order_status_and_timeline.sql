-- Extend order status to include confirmed and shipped
ALTER TABLE orders
  DROP CONSTRAINT IF EXISTS orders_status_check;

ALTER TABLE orders
  ADD CONSTRAINT orders_status_check CHECK (
    status IN ('pending','confirmed','processing','packed','shipped','delivered','cancelled')
  );

-- Rename placed -> pending via default update (old 'placed' rows kept compatible)
-- We keep 'placed' as an alias by widening the constraint
ALTER TABLE orders
...
-- Order timeline / status history log
...
-- Trigger: auto-insert timeline entry on order status change
